...
class Model {
    ...
    std::vector<int> facet_tex = {}; // texture index for uvs
    std::vector<std::pair<float, float>> vert_uv = {};
public:
    ...
    std::pair<float,float> uv(const int iface, const int nthvert) const; // uv coordinates
};